...
import { world, Position, Look, ChunkCoord, Renderable, RenderMesh, VoxelVolume, addGrowable, destroyChunkEntity } from '../core/ecs/components.js';
...
async function main() {
  ...
  let currentRenderMode = document.getElementById('render-select') ? document.getElementById('render-select').value : 'raster';
  ...
  async function buildWorld(storageType, terrainType, renderMode) {
    ...
    try {
      const stats = await pool.processAllChunks(
        (received, total) => {
          ui.setStatus(`Benchmarking: ${terrainType} - ${storageType.toUpperCase()}… (${received}/${total})`, received / total);
        },
        uploadChunkMesh,
        storageType,
        terrainType,
  ...
  engineAPI.register({
    ...
    handler: () => ({
      ...
      yaw: Look.yaw[player], pitch: Look.pitch[player]
    })
  });
  ...
  engineAPI.register({
    ...
    schema: { type: 'object', properties: {} },
    handler: () => ({
      fps: fpsDisplay, chunks: chunkEntities.length, workers: poolSize
    })
  });
  ...
  input.onMouseDown = (button) => {
    ...
    for (let i = 0; i < 50; i++) { 
        if (sideDistX < sideDistY && sideDistX < sideDistZ) {
            ...
            sideDistZ += deltaDistZ; mapZ += stepZ; side = 2;
        }
    ...
    if (hit) {
        if (button === 0) { // Klik Kiri = Hancurkan (Set jadi Udara = 0)
            renderer.editVoxel(mapX, mapY, mapZ, 0);
        } else if (button === 2) { // Klik Kanan = Bangun (Dirt = 2)
            let bx = mapX; let by = mapY; let bz = mapZ;
            if (side === 0) bx -= stepX;
  ...
  function frame(now) {
    try {
      ...
      fpsAcc += dt; fpsFrames++;
      if (fpsAcc >= 0.4) { fpsDisplay = Math.round(fpsFrames / fpsAcc); fpsAcc = 0; fpsFrames = 0; }
