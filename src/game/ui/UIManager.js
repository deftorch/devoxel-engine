export class UIManager {
  constructor() {
    ...
    window.addEventListener('unhandledrejection', (e) =>
      this.fail('Unhandled promise rejection:\n' + (e.reason?.stack || e.reason))
    );
  }
  ...
  updateHUD(fps, chunkCount, workerCount, cameraState, lastRemovedInfo, benchmarkStats, diagnostics) {
    let benchHTML = '';
    if (benchmarkStats) {
      benchHTML =
        `<b>Storage:</b> ${benchmarkStats.type.toUpperCase()} &middot; ` +
    ...
    this.hud.innerHTML =
      ...
      (lastRemovedInfo
        ? `<b style="color:#7fffb0">✓ Entity ${lastRemovedInfo.eid} dihapus, buffer di-destroy() lewat observer</b><br>`
        : '') +
      ...
      (diagnostics
        ? `<br><b>RT:</b> used:${diagnostics.globalBrickCount - diagnostics.freeBrickListLength} ` +
          `/ alloc:${diagnostics.globalBrickCount} free:${diagnostics.freeBrickListLength}` +
          ` &nbsp; sample:${diagnostics.freeBrickListSample.join(',')}`
        : '');
  }
}
