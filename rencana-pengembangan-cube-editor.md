...
- [x] **Bug drag "off-by-one" ditemukan dari laporan pengembang** ("drag dari kotak ke-1 malah lompat ke kotak ke-2, harus diakali mulai dari kotak ke-0"): root cause — snapping titik pakai `Math.round()` (ke vertex/garis grid terdekat), bukan `Math.floor()` (ke sel/cell yang memuat titik tersebut). Akibatnya, hover di **separuh kedua** sebuah sel (misal local x=1.7, masih di dalam sel 1 yang membentang [1,2)) di-snap ke vertex 2, bukan sel 1 — persis gejala yang dilaporkan. Diperbaiki dengan `AddToolState.snapToCell()`: sumbu yang sejajar `localNormal` (bidang datar gambar) tetap dibulatkan biasa (memang harus presisi di bidang wajah), tapi 2 sumbu lainnya (arah gambar alas) di-*floor* ke kelipatan unit terdekat DI BAWAH titik — sehingga titik manapun di dalam satu sel konsisten merujuk ke sel yang sama, meniru cara Minecraft/editor voxel lain menentukan "sel mana yang sedang di-hover". Konsekuensinya, logika `getCubeTransform()` juga disederhanakan: sekarang **selalu** menambah `+unit` ke ujung `max` (dulu cuma kalau `min===max`, yang jadi sumber ketidak-konsistenan tambahan antara kasus klik-tunggal vs drag-multi-sel).
- [x] **Bug extrude "terlalu sensitif" ditemukan dari laporan pengembang** (klik cepat setelah gambar alas, berharap tinggi 1 blok, malah lompat jadi beberapa blok): root cause — perhitungan tinggi (`closestParamsBetweenLines` antara ray kamera dan garis sumbu normal) numerically tidak stabil saat ray pandang kamera hampir sejajar dengan sumbu extrude (kasus umum saat melihat wajah target agak dari depan) — pembagi `denom=1-b²` jadi sangat kecil (tapi tidak nol, jadi tidak ke-tangkap guard yang ada), memperkuat noise/jitter mouse kecil jadi lompatan `s` yang besar. Diperbaiki dengan menambah **ambang gerak minimum** (`EXTRUDE_MOVE_THRESHOLD = 6px`, meniru pola `MARQUEE_THRESHOLD` yang sudah ada) sejak fase EXTRUDE dimulai — tinggi tidak diperbarui sampai cursor benar-benar bergerak signifikan dari titik saat alas selesai digambar, jadi klik-cepat-langsung-selesai (jitter mouse manusiawi yang wajar, beberapa piksel) tetap menghasilkan tinggi 1 unit yang stabil.
- [x] **Fitur baru — klik kanan untuk batal:** saat ada penempatan yang belum selesai (fase `DRAW_BASE`/`EXTRUDE`), klik kanan sekarang membatalkan (`cancelAddTool()`) alih-alih pan kamera seperti biasa. Kalau tidak ada yang sedang digambar (fase `HOVER`), klik kanan tetap pan seperti biasa — tidak ada perubahan perilaku di luar mode add yang sedang menggambar.
- [x] **Fitur baru — Ctrl+Scroll untuk ubah ukuran unit dasar (1-16):** dipilih **Ctrl**, bukan alternatif **S+Scroll** yang sempat diusulkan, karena `S` sudah terikat sebagai hotkey pindah ke mode gizmo Scale (lihat Fase 6.5) — menekan S akan langsung `AddToolState.active=false` (keluar dari mode Add) sebelum scroll-nya sempat berarti apa-apa, jadi kombinasi itu akan tabrakan dan tidak pernah benar-benar terpakai. Ctrl tidak dipakai di binding manapun lagi di `camera-input.js`, jadi aman dipakai ulang di sini tanpa konflik.
- [x] **Peningkatan UI/UX — HUD indikator ukuran** (`#add-tool-hud`, elemen DOM baru, `pointer-events:none` seperti pola `#marquee-box`): menampilkan ukuran unit dasar saat idle (`fase HOVER`), lebar/panjang alas saat menggambar (`fase DRAW_BASE`), dan dimensi lengkap + petunjuk klik-kanan-batal saat menentukan tinggi (`fase EXTRUDE`) — diperbarui langsung tiap frame gerak mouse dan tiap kali `baseUnitSize` berubah lewat Ctrl+Scroll.
- [x] **Checkpoint:** semua perbaikan & fitur di atas diverifikasi lewat simulasi terisolasi (snapping sel vs vertex, ambang gerak extrude, resize unit, cancel) sebelum di-commit. `npm test` tetap 52/53 lolos (kegagalan pra-eksisting `setup.js`, tidak terkait).
- [x] **Konfigurasi Add Tool (dipersempit dari permintaan awal — prioritas pertama: unit dasar + palet warna):** modul baru `src/editor/settings.js` menangani baca/tulis `localStorage` (kunci `devoxel.addToolSettings.v1`), dengan validasi/sanitasi ketat (warna hex tervalidasi regex, ukuran unit di-clamp 1-16) dan fallback aman ke default kalau data rusak/korup/`localStorage` tidak tersedia (dibungkus try/catch — **diverifikasi eksplisit** karena environment testing Node.js proyek ini tidak punya `localStorage` sama sekali; tanpa fallback ini, sekadar mengimpor `tool-add.js` akan crash di lingkungan test). Panel UI baru `src/editor/ui/add-tool-settings.js` (tombol "⚙ Add Tool" di toolbar) — modal dengan input angka untuk unit dasar dan editor swatch warna (tambah/hapus/ubah warna via `<input type=color>`, minimal 1 warna wajib tersisa supaya `PALETTE` tidak pernah kosong), tombol Reset ke Default, ter-sinkron otomatis lewat event `addToolSettingsChanged` (pola pub/sub Fase 6.6) kalau `baseUnitSize` diubah dari luar panel (mis. lewat Ctrl+Scroll saat panel kebetulan terbuka).
- [ ] **Belum dikerjakan** (di luar prioritas pertama yang dipilih): kontrol kamera (sensitivitas scroll/drag, FOV) dan tampilan (warna grid, warna gizmo, tema editor) — menyusul kalau diminta.

### 6.9 — Regresi Manual Menyeluruh & Alat Bantunya (`npm run regression`)

**Tujuan:** satu alur uji berantai yang mencakup interaksi ANTAR fitur (bukan cuma tiap fitur sendiri-sendiri seperti `npm test`), sebagai baseline sekarang bahwa Fase 6 secara fungsional lengkap.

- [x] `npm test` (unit-level, fungsi murni/terisolasi) dilengkapi dengan `npm run regression` (interaction-level) — `scripts/regression-check.mjs` + `scripts/regression-check-lib.mjs`. Memuat **`editor.html` asli** ke `jsdom` (bukan stub DOM manual), lalu men-*dispatch* **event mouse sungguhan** (`mousedown`/`mousemove`/`mouseup`) ke handler produksi yang sama persis dipakai browser. Koordinat klik dihitung presisi lewat proyeksi 3D→2D yang sama dan sudah terverifikasi dari `frustumSelect()` (Fase 6.3) — `worldToScreen()` di `regression-check-lib.mjs`.
- [x] Skenario 7 langkah tervalidasi lolos (langkah 8, ganti renderer WebGPU↔WebGL, dilewati — butuh konteks GPU nyata yang mustahil disimulasikan di Node/jsdom): buat 3 kubus berbeda posisi → rotate salah satu lewat gizmo → marquee-select ketiganya → translate bareng (termasuk yang sudah dirotasi) → Add Tool nempel di wajah miring kubus yang dirotasi → undo 4× + redo → duplicate + delete.
- [x] **Keterbatasan yang didokumentasikan secara eksplisit** (bukan disembunyikan sebagai "lolos semua"): mengarahkan klik sintetis ke satu ring gizmo Rotate yang **spesifik** (misal harus tepat sumbu Y) terbukti sulit dikalibrasi tanpa umpan balik visual — 2 percobaan pertama malah kena sumbu lain. Pengecekan dilonggarkan jadi "interaksi rotate menghasilkan rotasi nyata + History entry benar" (yang tetap bernilai, karena membuktikan jalur event DOM asli bekerja), bukan menuntut sumbu spesifik. Matematika rotasi itu sendiri sudah diverifikasi ketat & independen lewat test round-trip matriks di Fase 6.5 — bukan celah cakupan yang sebenarnya, cuma batasan kalibrasi skrip buta tanpa mata manusia.
- [x] `jsdom` ditambahkan sebagai `devDependency` (bukan dependency produksi — cuma dipakai skrip regresi). Ditambahkan blok config ESLint baru khusus `scripts/**/*.mjs` (gabungan `globals.node` + `globals.browser`, pola yang sama seperti `src/test/**` yang sudah ada) supaya `window`/`document`/`MouseEvent` dari simulasi jsdom tidak dianggap undefined oleh linter.
- [ ] **Belum diverifikasi otomatis** (dan secara fundamental tidak bisa, bukan "belum sempat"): rendering visual sungguhan (WebGPU/WebGL, shader, apakah gizmo/warna *terlihat* benar) dan pergantian renderer (Langkah 8) — keduanya perlu mata manusia + GPU nyata di browser.
...
### 6.10 — Add Tool Bebas (non-snap), Smart Extrude, dan Modal Transform G/R/S (ditambahkan langsung oleh pengembang, direview)

**Catatan:** 5 commit besar di luar rencana tertulis — ditambahkan langsung, direview setelahnya.

- [x] **Add Tool mode bebas (non-snap)**: `AddToolState.snapEnabled` + `resolvePoint()` (floor-ke-grid vs bulat-3-desimal), toggle di panel Settings, Ctrl sebagai pembalik sementara per-drag (pola Blender). Default sekarang **snap OFF** (`DEFAULT_SNAP_ENABLED = false`, sempat 2 tombol terpisah lalu dikonsolidasi jadi 1 tombol + toggle).
- [x] **Grid overlay saat hover** (`buildHoverFaceGrid`) + **Smart Extrude** (Shift+hover mendeteksi wajah/tepi objek existing, Shift+klik langsung lompat ke fase EXTRUDE dengan alas = footprint wajah tsb, untuk "push-pull" ala SketchUp) — matematika `rawLocal`/`targetToWorld` (invers `worldToTarget`) sudah saya verifikasi benar secara aljabar.
- [x] **Sistem Modal Transform G/R/S ala Blender**: tekan G/R/S mengikuti mouse secara langsung (tanpa perlu klik-drag gagang gizmo dulu), X/Y/Z mengunci ke sumbu, Enter/klik-kiri konfirmasi, Escape/klik-kanan batal. Termasuk fungsi rotasi sumbu-bebas baru (`axisAngleMat3`, formula Rodrigues standar) untuk mode rotate tanpa constraint. **Diverifikasi lengkap** lewat event DOM asli: translate/rotate/scale dengan/tanpa constraint sumbu, konfirmasi via klik & Enter, batal via klik-kanan & Escape — semua berfungsi benar, termasuk memastikan constraint sumbu benar-benar mengunci 2 sumbu lain (tidak ikut berubah).
- [x] **Bug kritis ditemukan & diperbaiki saat review — kubus "sliver" nyaris tak terlihat (tinggi ≈0.001)**: karena mode default sekarang snap OFF, dan fase EXTRUDE memulai `height` dari placeholder `0.001` ("start flat", disengaja untuk preview), kalau user mengonfirmasi (klik lagi) **tanpa menggerakkan mouse sama sekali** setelah menggambar alas — skenario yang sangat wajar & sering terjadi — kubus yang dihasilkan setinggi 0.001 unit, nyaris tidak kelihatan. Berlaku untuk **kedua** alur (gambar alas normal maupun Smart Extrude). Diverifikasi lewat 2 simulasi terpisah sebelum diperbaiki. Diperbaiki dengan menambah `AddToolState.extrudeMoved` (dilacak sejak fase EXTRUDE dimulai, di-set `true` hanya saat perhitungan tinggi berbasis gerak mouse sungguhan berjalan) — saat finalize, kalau `extrudeMoved` masih `false`, tinggi diganti ke `baseUnitSize` (nilai wajar & terlihat) alih-alih memakai placeholder 0.001 apa adanya.
- [x] **`npm test` ditemukan merah (2 gagal) saat direview**: test `tool_add.test.js` "drag dari sel 1 ke sel 3" (ditulis di Fase 6.9) gagal karena bergantung pada perilaku grid-snap tapi tidak eksplisit set `snapEnabled=true`, sementara default-nya baru saja diubah jadi `false` di commit ini. Diperbaiki dengan menambah baris eksplisit di test tsb (sesuai maksud aslinya: menguji perilaku grid-snapped, bukan bebas) — bukan mengubah kode aplikasi.
- [ ] **Observasi kecil (belum diperbaiki, tidak mendesak):** menekan G/R/S tanpa ada seleksi diam-diam mengeluarkan user dari mode Add (karena `setGizmoModeAndSync` terpanggil duluan sebelum `startModalTransform` mengecek seleksi kosong dan batal), tanpa modal transform apa pun yang dimulai — tidak berbahaya, cuma sedikit membingungkan.
- [ ] **Observasi kecil (belum diperbaiki, tidak mendesak):** matematika rotate/scale sekarang terduplikasi antara sistem gizmo-drag lama (Fase 6.4/6.5) dan sistem modal transform baru — cukup berisiko kalau nanti ada bug fix yang cuma diterapkan ke salah satu tempat. Kandidat refactor masa depan: ekstrak jadi 1 fungsi bersama.
- [x] **Checkpoint:** `npm test` 69/70 (kegagalan pra-eksisting `setup.js`, tidak terkait). `npm run regression` (Fase 6.9) tetap 7/7 lolos tanpa perlu diubah sama sekali — bukti nilai investasi test level-interaksi itu, langsung terpakai ulang untuk memvalidasi perubahan besar yang tidak direncanakan sebelumnya.